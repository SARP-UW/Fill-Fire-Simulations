...
%
% Syntax:
%   mdot_HEM = get_mass_flow_HEM_N2O(Cd_i, A_exit_i, rho_i_out, H_tank, H_i_out)
%
% Description:
...
%   H_i_out - N2O injector outlet enthalpy of two-phase mix (J/kg)
%
% Outputs:
%   mdot_HEM - N2O mass flow rate via HEM model (kg/s)

function mdot_HEM = get_mass_flow_HEM_N2O(Cd_i, A_exit_i, rho_i_out, H_tank, H_i_out)
    mdot_HEM = Cd_i * A_exit_i * rho_i_out * sqrt(2 * (H_tank - H_i_out));
end
